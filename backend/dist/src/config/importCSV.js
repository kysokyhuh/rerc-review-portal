...
function mapFunding(funding) {
    ...
    if (f.includes("rgmo") && !f.includes("non-rgmo"))
...
function mapStatus(status) {
    ...
    if (s.includes("cleared") || s.includes("exempted")) {
        return { submissionStatus: client_1.SubmissionStatus.CLOSED, decision: client_1.ReviewDecision.APPROVED };
    }
    if (s.includes("withdrawn")) {
        return { submissionStatus: client_1.SubmissionStatus.WITHDRAWN, decision: null };
    }
    return { submissionStatus: client_1.SubmissionStatus.RECEIVED, decision: null };
}
// Map PHREB research type
function mapResearchType(type) {
    const t = type?.toLowerCase().trim() || "";
    if (t.includes("biomedical"))
        return enums_1.ResearchTypePHREB.BIOMEDICAL;
    if (t.includes("social") || t.includes("behavioral"))
        return enums_1.ResearchTypePHREB.SOCIAL_BEHAVIORAL;
    if (t.includes("public health") || t.includes("epidemiolog"))
        return enums_1.ResearchTypePHREB.PUBLIC_HEALTH;
    if (t.includes("clinical"))
        return enums_1.ResearchTypePHREB.CLINICAL_TRIAL;
    if (t !== "" && t !== "others")
        return enums_1.ResearchTypePHREB.OTHER;
    return null;
}
// Map honorarium status
function mapHonorariumStatus(status) {
    const s = status?.toLowerCase().trim() || "";
    if (s.includes("paid"))
        return enums_1.HonorariumStatus.PAID;
    if (s.includes("processing"))
        return enums_1.HonorariumStatus.PROCESSING;
    if (s.includes("pending"))
        return enums_1.HonorariumStatus.PENDING;
    return enums_1.HonorariumStatus.NOT_APPLICABLE;
}
async function importCSV() {
    console.log("🚀 Starting CSV import...\n");
    // Read CSV file
    const csvPath = path.join(__dirname, "../../../[Intern Copy] RERC Protocol Database 2024 - 2024 Submission.csv");
    ...
    for (const panelName of ["Panel 1", "Panel 2", "Panel 3"]) {
        let panel = await prismaClient_1.default.panel.findFirst({
            where: { name: panelName, committeeId: committee.id },
        });
        if (!panel) {
            panel = await prismaClient_1.default.panel.create({
                data: {
                    name: panelName,
    ...
    for (let i = 0; i < dataRows.length; i++) {
        ...
        try {
            ...
            let projectStatus = client_1.ProjectStatus.ACTIVE;
            if (submissionStatus === client_1.SubmissionStatus.CLOSED) {
                projectStatus = client_1.ProjectStatus.CLOSED;
            }
            else if (submissionStatus === client_1.SubmissionStatus.WITHDRAWN) {
                projectStatus = client_1.ProjectStatus.WITHDRAWN;
            }
            ...
            let submission = await prismaClient_1.default.submission.findFirst({
                where: { projectId: project.id, submissionType: client_1.SubmissionType.INITIAL },
            });
            if (!submission) {
                ...
                submission = await prismaClient_1.default.submission.update({
            ...
            const createReviewerAndReview = async (name, role, honorarium) => {
                if (!name || name.trim() === "")
                    return;
                ...
                if (!reviewerId) {
                    ...
                    const reviewer = await prismaClient_1.default.user.upsert({
                        where: { email },
                ...
                if (!existingReview) {
                    await prismaClient_1.default.review.create({
                        data: {
                            ...
                            isPrimary: role === enums_1.ReviewerRoleType.SCIENTIST,
                            decision: decision,
            ...
            await createReviewerAndReview(scientistReviewer, enums_1.ReviewerRoleType.SCIENTIST, honorariumStatus);
            await createReviewerAndReview(layReviewer, enums_1.ReviewerRoleType.LAY, honorariumStatus);
            await createReviewerAndReview(independentConsultant, enums_1.ReviewerRoleType.INDEPENDENT_CONSULTANT, honorariumStatus);
            imported++;
            if (imported % 20 === 0) {
